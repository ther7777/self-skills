...
def load_research_config(config_path: str) -> Dict:
    ...
    try:
        ...
        # 转换 topics 格式到 research_domains 格式（兼容新旧配置）
        if 'topics' in config and 'research_domains' not in config:
            research_domains = {}
            for topic in config['topics']:
                domain_name = topic.get('name', topic.get('id', 'unknown'))
                research_domains[domain_name] = {
                    'keywords': topic.get('keywords', []),
                    'arxiv_categories': topic.get('categories', []),
                    'priority': topic.get('priority', 5)
                }
            config['research_domains'] = research_domains
            logger.info("Converted %d topics to research_domains format", len(config['topics']))
